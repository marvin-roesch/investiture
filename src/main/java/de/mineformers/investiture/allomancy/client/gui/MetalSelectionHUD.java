...
{
    public static final String[] METALS = {"bronze", "brass", "copper", "zinc", "tin", "iron", "pewter", "steel", "duralumin", "nicrosil", "aluminium", "chromium", "gold", "cadmium", "electrum", "bendalloy"};
    public static final ResourceLocation[] METAL_TEXTURES = (ResourceLocation[]) Arrays.stream(METALS).map(m -> new ResourceLocation(Allomancy.DOMAIN, "textures/metals/" + m + ".png")).toArray(ResourceLocation[]::new);
    ...
    private final Shader wheelShader = new Shader(new ResourceLocation(Allomancy.DOMAIN, "metal_wheel"), new ResourceLocation(Allomancy.DOMAIN, "metal_wheel"));
    ...
    public void onRenderOverlay(RenderGameOverlayEvent.Post event)
    {
        ...
        if (event.type != RenderGameOverlayEvent.ElementType.ALL || !display) return;
    ...
    private void drawMist(int centreX, int centreY)
    {
        ...
        wr.pos(centreX, centreY, 0).color(1, 1, 1, 0.3f).endVertex();
        for (int i = 100; i >= 0; i--) {
            double angle = PI * 2 * i / 100;
            wr.pos(centreX + cos(angle) * 60, centreY + sin(angle) * 60, 0).color(1, 1, 1, 0.3f).endVertex();
        }
        tess.draw();
        ...
        wr.pos(centreX + cos(0) * 60, centreY + sin(0) * 60, 0).color(1, 1, 1, 0.3f).endVertex();
        for (int i = -1; i <= 99; i++) {
            double angle0 = PI * 2 * i / 100;
            double angle1 = PI * 2 * (i + 1) / 100;
            wr.pos(centreX + cos(angle1) * 60, centreY + sin(angle1) * 60, 0).color(1, 1, 1, 0.3f).endVertex();
            wr.pos(centreX + cos(angle0) * 90, centreY + sin(angle0) * 90, 0).color(1, 1, 1, 0f).endVertex();
        }
        tess.draw();
    ...
    private void drawMetalIcons(int centreX, int centreY)
    {
        ...
        for (int i = 0; i < METALS.length / 2; i++) {
            ...
            iconShader.setUniform("backColour", burner.isBurning(innerMetal) ? new Vec3(205 / 255f, 43 / 255f, 0) : new Vec3(0.1f, 0.1f, 0.1f));
            ...
            Rendering.drawRectangle(centreX + (int) (cos(angle) * 40) - 8, centreY - (int) (sin(angle) * 40) - 8, 0, 0, 1, 1, 16, 16);

            Metal outerMetal = Metals.get(METALS[i * 2 + 1]).get();
            iconShader.setUniformBool("hovered", hoveredMetal.orNull() == outerMetal);
            // Change the main colour of the icon if the metal is burning
            iconShader.setUniform("backColour", burner.isBurning(outerMetal) ? new Vec3(205 / 255f, 43 / 255f, 0) : new Vec3(0.1f, 0.1f, 0.1f));
            ...
            Rendering.drawRectangle(centreX + (int) (cos(angle) * 75) - 8, centreY - (int) (sin(angle) * 75) - 8, 0, 0, 1, 1, 16, 16);
        }
